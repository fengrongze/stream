 ...
 * @Description 月数据查询统计mapper
 */
@Repository
public class CstStreamMonthStatisticsMapper<T extends CSTData> {
    ...
    public T findMonthTransforByRowKey(String tableName, String familyName, String rowKey,String columns[], Class<?> clazz) {
    ...
    public T findMonthTransforByRowKey(String tableName, Map familyQulifiers, String rowKey, String columns[], Class<?> clazz) {
        return (T) hbaseTemplate.get(tableName, rowKey,  (result, rowNum) ->
                new HbaseFindBuilder(familyQulifiers, clazz).build(result)).fetch();
    }

    public void putMonthTransfor(String tableName,String family,String rowKey, T data) {
        hbaseTemplate.execute(tableName, (table) -> {
            HbasePutBuilder<AmMonthTransfor> hbasePutBuilder = new HbasePutBuilder(family,rowKey.getBytes(),data);
            table.put(hbasePutBuilder);
            return true;
        });
    }

    public <T extends CSTData> List<T> findMonthTransforByScan(String tableName, Map<String, String[]> familyMap, String fromRowKey, String toRowKey, String[] allColumns, Class<?> clazz) {
        Scan scan = new Scan();
        scan.setCaching(200);
    ...
    public <T extends CSTData> List<T> findMonthTransforByRowKeys(String tableName, Map<String, String[]> familyMap, List<String> rowKeys, Class<?> clazz) {
