...
/**
 * @author Johnney.Chiu
 * create on 2018/3/12 16:13
 * @Description 小时数据持久化处理
 * @title
 */
public class HourDataPersistBolt  extends BaseBasicBolt {
    private static final long serialVersionUID = 4613552968581119118L;
    ...
    public HourDataPersistBolt(Properties prop, boolean forceLoad) {
        this.prop = prop;
    ...
    public void prepare(Map stormConf, TopologyContext context) {
        ...
        logger = LoggerFactory.getLogger(HourDataPersistBolt.class);
    ...
    public void execute(Tuple tuple, BasicOutputCollector collector) {
        ...
        switch (tuple.getSourceStreamId()){
            case StreamKey.AmStream.AM_HOUR_BOLT_S:
                dataPersist(msg, StrategyChoose.generateStrategy(prop.getProperty(PropKey.DEAL_STRATEGY), httpUtils,
                        ...
                        AmHourTransfor.class), CstConstants.TIME_SELECT.HOUR);
                break;
            case StreamKey.DeStream.DE_HOUR_BOLT_S:
                dataPersist(msg, StrategyChoose.generateStrategy(prop.getProperty(PropKey.DEAL_STRATEGY), httpUtils,
                        prop.getProperty("url_base"), HttpURIUtil.DE_HOUR_SAVE,
                        connection, HBaseTable.HOUR_STATISTICS.getTableName(), HBaseTable.HOUR_STATISTICS.getFirstFamilyName(),
                        HbaseColumn.HourStatisticsCloumn.deHourColumns,
                        DeHourTransfor.class), CstConstants.TIME_SELECT.HOUR);
                break;

            case StreamKey.TraceStream.TRACE_HOUR_BOLT_S:
                dataPersist(msg, StrategyChoose.generateStrategy(prop.getProperty(PropKey.DEAL_STRATEGY), httpUtils,
                        prop.getProperty("url_base"), HttpURIUtil.TRACE_HOUR_SAVE,
                        connection, HBaseTable.HOUR_STATISTICS.getTableName(), HBaseTable.HOUR_STATISTICS.getFirstFamilyName(),
                        HbaseColumn.HourStatisticsCloumn.traceHourColumns,
                        TraceHourTransfor.class), CstConstants.TIME_SELECT.HOUR);
                break;
            case StreamKey.TraceDeleteStream.TRACE_DELETE_HOUR_BOLT_S:
                dataPersist(msg, StrategyChoose.generateStrategy(prop.getProperty(PropKey.DEAL_STRATEGY), httpUtils,
                        prop.getProperty("url_base"), HttpURIUtil.TRACE_DELETE_HOUR_SAVE,
                        connection, HBaseTable.HOUR_STATISTICS.getTableName(), HBaseTable.HOUR_STATISTICS.getFirstFamilyName(),
                        HbaseColumn.HourStatisticsCloumn.traceDeleteHourColumns,
                        TraceDeleteHourTransfor.class), CstConstants.TIME_SELECT.HOUR);
                break;
            case StreamKey.VoltageStream.VOLTAGE_HOUR_BOLT_S:
                dataPersist(msg, StrategyChoose.generateStrategy(prop.getProperty(PropKey.DEAL_STRATEGY), httpUtils,
                        prop.getProperty("url_base"), HttpURIUtil.VOLTAGE_HOUR_SAVE,
                        connection, HBaseTable.HOUR_STATISTICS.getTableName(), HBaseTable.HOUR_STATISTICS.getFirstFamilyName(),
                        HbaseColumn.HourStatisticsCloumn.voltageHourColumns,
                        VoltageHourTransfor.class), CstConstants.TIME_SELECT.HOUR);
                break;
        }
    }
