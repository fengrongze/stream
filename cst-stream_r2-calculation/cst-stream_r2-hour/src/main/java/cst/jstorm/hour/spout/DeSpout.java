...
public class DeSpout extends BaseRichSpout {
    private static final Logger logger = LoggerFactory.getLogger(DeSpout.class);
    ...
    public DeSpout(Properties prop, boolean forceLoad) {
        this.prop = prop;
    ...
    public void nextTuple() {
        ...
        try {
            ...
            for (ConsumerRecord<String, byte[]> record : records) {
                goDe(record.value(),collector);
            }
            logger.debug("de topic deal time:{}", (System.currentTimeMillis() - start));
        }catch (Exception e){
            logger.error("de consumer hour data error",e);
        }
    }
    ...
    public void declareOutputFields(OutputFieldsDeclarer declarer) {
        // TODO Auto-generated method stub
        declarer.declareStream(StreamKey.DeStream.DE_HOUR_BOLT_F, new Fields(new String[]{
    ...
    private void initConsumer() {
        this.consumer = KafkaConsumerUtil.buildKafkaConsumer(prop, OtherKey.GroupTail.DE_HOUR);
        this.consumer.subscribe(Arrays.asList( prop.getProperty(KafkaTopicKey.DE_TOPIC)));
    }
