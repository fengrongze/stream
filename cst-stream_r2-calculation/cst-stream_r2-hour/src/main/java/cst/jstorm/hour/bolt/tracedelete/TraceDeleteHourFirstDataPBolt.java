 ...
 * @title
 */
public class TraceDeleteHourFirstDataPBolt extends BaseBasicBolt {
    private static final long serialVersionUID = 3340348381610451675L;
    ...
    public TraceDeleteHourFirstDataPBolt(Properties prop, boolean forceLoad) {
        this.prop = prop;
    ...
    public void prepare(Map stormConf, TopologyContext context) {
        ...
        logger = LoggerFactory.getLogger(TraceDeleteHourFirstDataPBolt.class);
    ...
    public void execute(Tuple tuple, BasicOutputCollector collector) {
        ...
        if(!StreamKey.TraceDeleteStream.TRACE_DELETE_HOUR_BOLT_FIRST_DATA.equals(tuple.getSourceStreamId()))
            return;
        firstDataPersist(msg, StrategyChoose.generateStrategy(prop.getProperty(PropKey.DEAL_STRATEGY),
                httpUtils, prop.getProperty("url_base"), HttpURIUtil.TRACE_DELETE_HOUR_SOURCE_SAVE,
                connection, HBaseTable.HOUR_FIRST_ZONE.getTableName(),
                HBaseTable.HOUR_FIRST_ZONE.getFirstFamilyName(),
                HbaseColumn.HourSourceColumn.traceDeleteHourColumns,
                TraceDeleteHourSource.class
        ),StreamTypeDefine.TRACE_DELETE_TYPE, CstConstants.TIME_SELECT.HOUR);
    }
