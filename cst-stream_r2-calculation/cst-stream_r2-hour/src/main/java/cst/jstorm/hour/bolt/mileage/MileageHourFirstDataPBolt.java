 ...
 * @title
 */
public class MileageHourFirstDataPBolt extends BaseBasicBolt {
    ...
    public MileageHourFirstDataPBolt(Properties prop, boolean forceLoad) {
        this.prop = prop;
    ...
    public void prepare(Map stormConf, TopologyContext context) {
        ...
        logger = LoggerFactory.getLogger(MileageHourFirstDataPBolt.class);
    ...
    public void execute(Tuple tuple, BasicOutputCollector collector) {
        ...
        if (!StreamKey.MileageStream.MILEAGE_HOUR_BOLT_FIRST_DATA.equals(tuple.getSourceStreamId())) {
            return;
        }

        firstDataPersist(msg, StrategyChoose.generateStrategy(prop.getProperty(PropKey.DEAL_STRATEGY),
                httpUtils, prop.getProperty("url_base"), HttpURIUtil.MILEAGE_HOUR_SOURCE_SAVE,
                connection, HBaseTable.HOUR_FIRST_ZONE.getTableName(),
                HBaseTable.HOUR_FIRST_ZONE.getFirstFamilyName(),
                HbaseColumn.HourSourceColumn.mileageHourColumns,
                MileageHourSource.class
        ), StreamTypeDefine.MILEAGE_TYPE, CstConstants.TIME_SELECT.HOUR);
    }
