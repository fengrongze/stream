 ...
 * @title
 */
public class ObdHourFirstDataPBolt extends BaseBasicBolt {
    ...
    public ObdHourFirstDataPBolt(Properties prop, boolean forceLoad) {
        this.prop = prop;
    ...
    public void prepare(Map stormConf, TopologyContext context) {
        ...
        logger = LoggerFactory.getLogger(ObdHourFirstDataPBolt.class);
    ...
    public void execute(Tuple tuple, BasicOutputCollector collector) {
        ...
        if (!StreamKey.ObdStream.OBD_HOUR_BOLT_FIRST_DATA.equals(tuple.getSourceStreamId())) {
            return;
        }

        firstDataPersist(msg, StrategyChoose.generateStrategy(prop.getProperty(PropKey.DEAL_STRATEGY),
                httpUtils, prop.getProperty("url_base"), HttpURIUtil.OBD_HOUR_SOURCE_SAVE,
                connection, HBaseTable.HOUR_FIRST_ZONE.getTableName(),
                HBaseTable.HOUR_FIRST_ZONE.getFirstFamilyName(),
                HbaseColumn.HourSourceColumn.obdHourColumns,
                ObdHourSource.class
        ), StreamTypeDefine.OBD_TYPE, CstConstants.TIME_SELECT.HOUR);
    }
