...
public class TraceDeleteGdcp3Spout extends BaseRichSpout {
	private static final Logger logger = LoggerFactory.getLogger(TraceDeleteGdcp3Spout.class);
	...
	public TraceDeleteGdcp3Spout(Properties prop, boolean forceLoad) {
		this.prop = prop;
	...
	public void nextTuple() {
		...
		try {
			...
			logger.debug("gdcp3 trace delete topic deal time:{}", (System.currentTimeMillis() - start));
		}catch (Exception e){
			logger.error("gpdc3 trace delete hour consumer data error",e);
		}
	}
	...
	public void declareOutputFields(OutputFieldsDeclarer declarer) {
		// TODO Auto-generated method stub
		declarer.declareStream(StreamKey.TraceDeleteStream.TRACE_DELETE_GDCP3_HOUR_BOLT_F, new Fields(new String[] {
				StreamKey.TraceDeleteStream.TRACE_DELETE_KEY_F, StreamKey.TraceDeleteStream.TRACE_DELETE_KEY_S }));
	}

	private void initConsumer() {
		this.consumer = KafkaConsumerUtil.buildStringKafkaConsumer(prop, OtherKey.Gdcp3GroupTail.TRACE_DELETE_HOUR);
		this.consumer.subscribe(Arrays.asList(prop.getProperty(KafkaTopicKey.Gdcp3TopicKey.GDCP3_TRACE_DELETE_TOPIC)));
	}
