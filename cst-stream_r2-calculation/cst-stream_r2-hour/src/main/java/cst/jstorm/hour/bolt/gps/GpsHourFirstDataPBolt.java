 ...
 * @title
 */
public class GpsHourFirstDataPBolt extends BaseBasicBolt {
    private static final long serialVersionUID = -7696254653383701613L;
    ...
    public GpsHourFirstDataPBolt(Properties prop, boolean forceLoad) {
        this.prop = prop;
    ...
    public void prepare(Map stormConf, TopologyContext context) {
        ...
        logger = LoggerFactory.getLogger(GpsHourFirstDataPBolt.class);
    ...
    public void execute(Tuple tuple, BasicOutputCollector collector) {
        ...
        if (!StreamKey.GpsStream.GPS_HOUR_BOLT_FIRST_DATA.equals(tuple.getSourceStreamId())) {
            return;
        }
        firstDataPersist(msg, StrategyChoose.generateStrategy(prop.getProperty(PropKey.DEAL_STRATEGY),
                httpUtils, prop.getProperty("url_base"), HttpURIUtil.GPS_HOUR_SOURCE_SAVE,
                connection, HBaseTable.HOUR_FIRST_ZONE.getTableName(),
                HBaseTable.HOUR_FIRST_ZONE.getFirstFamilyName(),
                HbaseColumn.HourSourceColumn.gpsHourColumns,
                GpsHourSource.class
        ), StreamTypeDefine.GPS_TYPE, CstConstants.TIME_SELECT.HOUR);
    }
