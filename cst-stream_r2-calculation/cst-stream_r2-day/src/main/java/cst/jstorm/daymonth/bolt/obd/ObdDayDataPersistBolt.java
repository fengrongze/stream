 ...
 * @title
 */
public class ObdDayDataPersistBolt extends BaseBasicBolt {
    ...
    public ObdDayDataPersistBolt(Properties prop, boolean forceLoad) {
        this.prop = prop;
    ...
    public void prepare(Map stormConf, TopologyContext context) {
        ...
        logger = LoggerFactory.getLogger(ObdDayDataPersistBolt.class);
    ...
    public void execute(Tuple tuple, BasicOutputCollector collector) {
        try {
            ...
            resultPersist(msg, StrategyChoose.generateStrategy(prop.getProperty(PropKey.DEAL_STRATEGY), httpUtils,
                    prop.getProperty("url_base"), HttpURIUtil.OBD_DAY_SAVE,
                    connection, HBaseTable.DAY_STATISTICS.getTableName(),
                    HBaseTable.DAY_STATISTICS.getFirstFamilyName(),
                    HbaseColumn.DayStatisticsCloumn.obdDayColumns,
                    ObdDayTransfor.class), StreamTypeDefine.OBD_TYPE, CstConstants.TIME_SELECT.DAY);

            //持久化完成,发送到后端处理
            collector.emit(StreamKey.ObdStream.OBD_DAY_PERSIST_S,new Values(msg,msgId, IntegrityDayCalcEnum.OBD.getValue()));
        } catch (Throwable e) {
            logger.warn("obd data persist warn ",e);
        }
    }
