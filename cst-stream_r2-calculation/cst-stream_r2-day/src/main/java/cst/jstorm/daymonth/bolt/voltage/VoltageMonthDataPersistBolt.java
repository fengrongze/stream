 ...
 * @Description voltage month persist
 * @title
 */
public class VoltageMonthDataPersistBolt extends BaseBasicBolt {
    ...
    public VoltageMonthDataPersistBolt(Properties prop, boolean forceLoad) {
        this.prop = prop;
    ...
    public void prepare(Map stormConf, TopologyContext context) {
        ...
        logger = LoggerFactory.getLogger(VoltageMonthDataPersistBolt.class);
    ...
    public void execute(Tuple tuple, BasicOutputCollector collector) {
        ...
        if (StringUtils.isBlank(msg))
            return;
        if(!StreamKey.VoltageStream.VOLTAGE_MONTH_BOLT_S.equals(tuple.getSourceStreamId()))
            return;
        resultPersist(msg, StrategyChoose.generateStrategy(prop.getProperty(PropKey.DEAL_STRATEGY), httpUtils,
                prop.getProperty("url_base"), HttpURIUtil.VOLTAGE_MONTH_SAVE,
                ...
                VoltageMonthTransfor.class), StreamTypeDefine.VOLTAGE_TYPE, CstConstants.TIME_SELECT.MONTH);
    }
