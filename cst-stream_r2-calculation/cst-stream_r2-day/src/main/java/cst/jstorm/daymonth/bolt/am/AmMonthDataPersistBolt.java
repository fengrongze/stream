 ...
 * @title
 */
public class AmMonthDataPersistBolt extends BaseBasicBolt {
    ...
    public AmMonthDataPersistBolt(Properties prop, boolean forceLoad) {
        this.prop = prop;
    ...
    public void prepare(Map stormConf, TopologyContext context) {
        ...
        logger = LoggerFactory.getLogger(AmMonthDataPersistBolt.class);
    ...
    public void execute(Tuple tuple, BasicOutputCollector collector) {
        ...
        if (StringUtils.isBlank(msg))
            return;
        if(!StreamKey.AmStream.AM_MONTH_BOLT_S.equals(tuple.getSourceStreamId()))
            return;
        resultPersist(msg, StrategyChoose.generateStrategy(prop.getProperty(PropKey.DEAL_STRATEGY), httpUtils,
                prop.getProperty("url_base"), HttpURIUtil.AM_MONTH_SAVE,
                connection, HBaseTable.MONTH_STATISTICS.getTableName(),
                HBaseTable.MONTH_STATISTICS.getThirdFamilyName(),
                HbaseColumn.MonthStatisticsCloumn.amMonthColumns,
                AmMonthTransfor.class), StreamTypeDefine.AM_TYPE, CstConstants.TIME_SELECT.MONTH);
    }
