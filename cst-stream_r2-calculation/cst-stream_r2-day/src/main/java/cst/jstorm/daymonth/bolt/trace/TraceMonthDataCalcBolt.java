...
import cst.jstorm.daymonth.calcalations.trace.TraceMonthCalcBiz;
 ...
 * @Description trace数据的月计算
 */
public class TraceMonthDataCalcBolt extends BaseBasicBolt {
    ...
    private TraceMonthCalcBiz traceMonthCalcBiz;

    public TraceMonthDataCalcBolt(Properties prop, boolean forceLoad) {
        this.prop = prop;
    ...
    public void prepare(Map stormConf, TopologyContext context) {
        ...
        logger = LoggerFactory.getLogger(TraceMonthDataCalcBolt.class);
        ...
        traceMonthCalcBiz = new TraceMonthCalcBiz();
    }

    @Override
    @SuppressWarnings("unchecked")
    public void execute(Tuple input, BasicOutputCollector collector) {
        if (!StreamKey.TraceStream.TRACE_MONTH_BOLT_F.equals(input.getSourceStreamId()))
            return;
        ...
        try {
            ...
            GeneralAccumulationStreamExecution<TraceDayTransfor, TraceMonthTransfor, TraceMonthCalcBiz> generalStreamExecution = new GeneralAccumulationStreamExecution<>()
                    .createJedis(jedis)
                    .createSpecialCalc(traceMonthCalcBiz)
                    .createSpecialSource(msg, StreamRedisConstants.MonthKey.MONTH_TRACE, DateTimeUtil.DEFAULT_DATE_MONTH);
            //如果从缓存中拿到不是空数据
            IHBaseQueryAndPersistStrategy<TraceMonthTransfor> iResultStrategy = StrategyChoose.generateStrategy(prop.getProperty(PropKey.DEAL_STRATEGY),
                    httpUtils, prop.getProperty("url_base"), HttpURIUtil.TRACE_MONTH_FIND,
                    connection, HBaseTable.MONTH_STATISTICS.getTableName(),
                    HBaseTable.MONTH_STATISTICS.getFifthFamilyName(),
                    HbaseColumn.MonthStatisticsCloumn.traceMonthColumns,
                    TraceMonthTransfor.class);
            ...
            if (CollectionUtils.isNotEmpty(persistValues)) {
                ...
                for (String str : persistValues)
                    collector.emit(StreamKey.TraceStream.TRACE_MONTH_BOLT_S, new Values(str, generalStreamExecution.gentMsgId()));
            }
    ...
    public void declareOutputFields(OutputFieldsDeclarer declarer) {
        declarer.declareStream(StreamKey.TraceStream.TRACE_MONTH_BOLT_S, new Fields(new String[] {
