 ...
 * @title
 */
public class MileageYearDataPersistBolt extends BaseBasicBolt {
    ...
    public MileageYearDataPersistBolt(Properties prop, boolean forceLoad) {
        this.prop = prop;
    ...
    public void prepare(Map stormConf, TopologyContext context) {
        ...
        logger = LoggerFactory.getLogger(MileageYearDataPersistBolt.class);
    ...
    public void execute(Tuple tuple, BasicOutputCollector collector) {
        ...
        if (StringUtils.isBlank(msg))
            return;
        if(!StreamKey.MileageStream.MILEAGE_YEAR_BOLT_S.equals(tuple.getSourceStreamId()))
            return;
        resultPersist(msg, StrategyChoose.generateStrategy(prop.getProperty(PropKey.DEAL_STRATEGY), httpUtils,
                prop.getProperty("url_base"), HttpURIUtil.MILEAGE_YEAR_SAVE,
                connection, HBaseTable.YEAR_STATISTICS.getTableName(),
                HBaseTable.YEAR_STATISTICS.getEighthFamilyName(),
                HbaseColumn.YearStatisticsCloumn.mileageYearColumns,
                MileageYearTransfor.class), StreamTypeDefine.MILEAGE_TYPE, CstConstants.TIME_SELECT.YEAR);
    }
