...
import cst.jstorm.daymonth.calcalations.de.DeYearCalcBiz;
 ...
 * @Description de数据的天计算
 */
public class DeYearDataCalcBolt extends BaseBasicBolt {
    ...
    private DeYearCalcBiz deYearCalcBiz;

    public DeYearDataCalcBolt(Properties prop, boolean forceLoad) {
        this.prop = prop;
    ...
    public void prepare(Map stormConf, TopologyContext context) {
        ...
        logger = LoggerFactory.getLogger(DeYearDataCalcBolt.class);
        ...
        deYearCalcBiz = new DeYearCalcBiz();
    ...
    public void execute(Tuple input, BasicOutputCollector collector) {
        if (!StreamKey.DeStream.DE_YEAR_BOLT_F.equals(input.getSourceStreamId()))
            return;
        ...
        try {
            ...
            GeneralAccumulationStreamExecution<DeDayTransfor, DeYearTransfor, DeYearCalcBiz> generalStreamExecution =
                    new GeneralAccumulationStreamExecution<>()
                    .createJedis(jedis)
                    .createSpecialCalc(deYearCalcBiz)
                    .createSpecialSource(msg, StreamRedisConstants.YearKey.YEAR_DE, DateTimeUtil.DEFAULT_DATE_YEAR);
            //如果从缓存中拿到不是空数据
            IHBaseQueryAndPersistStrategy<DeYearTransfor> iResultStrategy = StrategyChoose.generateStrategy(prop.getProperty(PropKey.DEAL_STRATEGY),
                    httpUtils, prop.getProperty("url_base"), HttpURIUtil.DE_YEAR_FIND,
                    connection, HBaseTable.YEAR_STATISTICS.getTableName(),
                    HBaseTable.YEAR_STATISTICS.getFourthFamilyName()
                    , HbaseColumn.YearStatisticsCloumn.deYearColumns,
                    DeYearTransfor.class);
            ...
            if(CollectionUtils.isNotEmpty(persistValues))
                for (String str : persistValues)
                    collector.emit(StreamKey.DeStream.DE_YEAR_BOLT_S, new Values(str, generalStreamExecution.gentMsgId()));
    ...
    public void declareOutputFields(OutputFieldsDeclarer declarer) {
        declarer.declareStream(StreamKey.DeStream.DE_YEAR_BOLT_S, new Fields(new String[] {
