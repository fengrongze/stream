...
public class OtherGdcp3Spout extends BaseRichSpout {
	...
	public void nextTuple() {
		...
		try {
			...
			for (ConsumerRecord<String,String> record : records){
				Gdcp3DataParseFromSource.goOther(record.value(), collector, map);
			}
	...
	public void declareOutputFields(OutputFieldsDeclarer declarer) {
		...
		declarer.declareStream(StreamKey.DormancyStream.DORMANCY_GDCP3_DAY_BOLT_F, new Fields(new String[] {
				StreamKey.DormancyStream.DORMANCY_KEY_F, StreamKey.DormancyStream.DORMANCY_KEY_S }));

	}

	private void initConsumer() {
		this.consumer = KafkaConsumerUtil.buildKafkaConsumer(prop, OtherKey.Gdcp3GroupTail.VOLTAGE_DAY);
