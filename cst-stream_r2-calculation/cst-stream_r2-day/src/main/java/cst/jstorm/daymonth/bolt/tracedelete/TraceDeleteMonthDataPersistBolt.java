 ...
 * @Description trace delete month persist
 * @title
 */
public class TraceDeleteMonthDataPersistBolt extends BaseBasicBolt {
    ...
    public TraceDeleteMonthDataPersistBolt(Properties prop, boolean forceLoad) {
        this.prop = prop;
    ...
    public void prepare(Map stormConf, TopologyContext context) {
        ...
        logger = LoggerFactory.getLogger(TraceDeleteMonthDataPersistBolt.class);
    ...
    public void execute(Tuple tuple, BasicOutputCollector collector) {
        ...
        if (StringUtils.isBlank(msg))
            return;
        if(!StreamKey.TraceDeleteStream.TRACE_DELETE_MONTH_BOLT_S.equals(tuple.getSourceStreamId()))
            return;
        resultPersist(msg, StrategyChoose.generateStrategy(prop.getProperty(PropKey.DEAL_STRATEGY), httpUtils,
                prop.getProperty("url_base"), HttpURIUtil.TRACE_DELETE_MONTH_SAVE,
                connection, HBaseTable.MONTH_STATISTICS.getTableName(),
                HBaseTable.MONTH_STATISTICS.getSixthFamilyName(),
                HbaseColumn.MonthStatisticsCloumn.traceDeleteMonthColumns,
                TraceDeleteMonthTransfor.class), StreamTypeDefine.TRACE_DELETE_TYPE, CstConstants.TIME_SELECT.MONTH);
    }
