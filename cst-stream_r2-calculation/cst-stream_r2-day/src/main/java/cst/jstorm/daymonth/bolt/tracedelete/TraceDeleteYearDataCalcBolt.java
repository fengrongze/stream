...
import cst.jstorm.daymonth.calcalations.tracedelete.TraceDeleteYearCalcBiz;
 ...
 * @Description trace delete数据的年计算
 */
public class TraceDeleteYearDataCalcBolt extends BaseBasicBolt {
    ...
    private TraceDeleteYearCalcBiz traceDeleteYearCalcBiz;

    public TraceDeleteYearDataCalcBolt(Properties prop, boolean forceLoad) {
        this.prop = prop;
    ...
    public void prepare(Map stormConf, TopologyContext context) {
        ...
        logger = LoggerFactory.getLogger(TraceDeleteYearDataCalcBolt.class);
        ...
        traceDeleteYearCalcBiz = new TraceDeleteYearCalcBiz();
    ...
    public void execute(Tuple input, BasicOutputCollector collector) {
        if (!StreamKey.TraceDeleteStream.TRACE_DELETE_YEAR_BOLT_F.equals(input.getSourceStreamId()))
            return;
        ...
        try {
            ...
            GeneralAccumulationStreamExecution<TraceDeleteDayTransfor, TraceDeleteYearTransfor, TraceDeleteYearCalcBiz> generalStreamExecution = new GeneralAccumulationStreamExecution<>()
                    .createJedis(jedis)
                    .createSpecialCalc(traceDeleteYearCalcBiz)
                    .createSpecialSource(msg, StreamRedisConstants.YearKey.YEAR_TRACE_DELETE, DateTimeUtil.DEFAULT_DATE_YEAR);
            //如果从缓存中拿到不是空数据
            IHBaseQueryAndPersistStrategy<TraceDeleteYearTransfor> iResultStrategy = StrategyChoose.generateStrategy(prop.getProperty(PropKey.DEAL_STRATEGY),
                    httpUtils, prop.getProperty("url_base"), HttpURIUtil.TRACE_DELETE_YEAR_FIND,
                    connection, HBaseTable.YEAR_STATISTICS.getTableName(),
                    HBaseTable.YEAR_STATISTICS.getSixthFamilyName(),
                    HbaseColumn.YearStatisticsCloumn.traceDeleteYearColumns,
                    TraceDeleteYearTransfor.class);
            ...
            if(CollectionUtils.isNotEmpty(persistValues))
                for (String str : persistValues)
                    collector.emit(StreamKey.TraceDeleteStream.TRACE_DELETE_YEAR_BOLT_S, new Values(str, generalStreamExecution.gentMsgId()));
    ...
    public void declareOutputFields(OutputFieldsDeclarer declarer) {
        declarer.declareStream(StreamKey.TraceDeleteStream.TRACE_DELETE_YEAR_BOLT_S, new Fields(new String[] {
