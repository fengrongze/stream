...
import cst.jstorm.daymonth.calcalations.gps.GpsMonthCalcBiz;
 ...
 * @Description gps数据的月计算
 */
public class GpsMonthDataCalcBolt extends BaseBasicBolt {
    ...
    private GpsMonthCalcBiz gpsMonthCalcBiz;

    public GpsMonthDataCalcBolt(Properties prop, boolean forceLoad) {
        this.prop = prop;
    ...
    public void prepare(Map stormConf, TopologyContext context) {
        ...
        logger = LoggerFactory.getLogger(GpsMonthDataCalcBolt.class);
        ...
        gpsMonthCalcBiz = new GpsMonthCalcBiz();
    ...
    public void execute(Tuple input, BasicOutputCollector collector) {
        if (!StreamKey.GpsStream.GPS_MONTH_BOLT_F.equals(input.getSourceStreamId()))
            return;
        ...
        try {
            ...
            GeneralAccumulationStreamExecution<GpsDayTransfor, GpsMonthTransfor, GpsMonthCalcBiz> generalStreamExecution =
                    new GeneralAccumulationStreamExecution<>()
                    .createJedis(jedis)
                    .createSpecialCalc(gpsMonthCalcBiz)
                    .createSpecialSource(msg, StreamRedisConstants.MonthKey.MONTH_GPS, DateTimeUtil.DEFAULT_DATE_MONTH);
            //如果从缓存中拿到不是空数据
            IHBaseQueryAndPersistStrategy<GpsMonthTransfor> iResultStrategy = StrategyChoose.generateStrategy(prop.getProperty(PropKey.DEAL_STRATEGY),
                    httpUtils, prop.getProperty("url_base"), HttpURIUtil.GPS_MONTH_FIND,
                    connection, HBaseTable.MONTH_STATISTICS.getTableName(),
                    HBaseTable.MONTH_STATISTICS.getSecondFamilyName()
                    , HbaseColumn.MonthStatisticsCloumn.gpsMonthColumns,
                    GpsMonthTransfor.class);
            ...
            if(CollectionUtils.isNotEmpty(persistValues))
                for (String str : persistValues)
                    collector.emit(StreamKey.GpsStream.GPS_MONTH_BOLT_S, new Values(str, generalStreamExecution.gentMsgId()));
    ...
    public void declareOutputFields(OutputFieldsDeclarer declarer) {
        declarer.declareStream(StreamKey.GpsStream.GPS_MONTH_BOLT_S, new Fields(new String[] {
