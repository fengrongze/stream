...
import cst.jstorm.daymonth.spout.gdcp3.*;
 ...
 * @title
 */
public class DayGdcp3TopoCreaterAdapter extends TopoCreateAdapter {

    protected BoltCreateAdapter boltCreateAdapter;

    public DayGdcp3TopoCreaterAdapter(TopologyBuilder builder, Properties props, BoltCreateAdapter boltCreateAdapter) {
        super(builder, props);
    ...
    protected void createSpout(String spout) {
        ...
        if (spout.contains("obd")) {
            ...
            builder.setSpout(ComponentNameEnum.OBD.getSpoutNameOther(), new ObdGdcp3Spout(props, false), obdSpoutNum == 0
                    ? defaultTaskNum : obdSpoutNum);
            builder.setSpout(ComponentNameEnum.ELECTRIC_OBD.getSpoutNameOther(), new ElectricObdGdcp3Spout(props, false), electricObdSpoutNum == 0
                    ? defaultTaskNum : electricObdSpoutNum);
        }
        if(spout.contains("gps"))
            builder.setSpout(ComponentNameEnum.GPS.getSpoutNameOther(), new GpsGdcp3Spout(props,false), gpsSpoutNum ==0
                ?defaultTaskNum:gpsSpoutNum);

        if(spout.contains("de"))
            builder.setSpout(ComponentNameEnum.DE.getSpoutNameOther(), new DeGdcp3Spout(props,false), deSpoutNum ==0
                ?defaultTaskNum:deSpoutNum);

        if(spout.contains("trace"))
            builder.setSpout(ComponentNameEnum.TRACE.getSpoutNameOther(), new TraceGdcp3Spout(props,false), traceSpoutNum==0
                ?defaultTaskNum:traceSpoutNum);

        if(spout.contains("trace_delete"))
            builder.setSpout(ComponentNameEnum.TRACE_DELETE.getSpoutNameOther(), new TraceDeleteGdcp3Spout(props,false), traceDeleteSpoutNum==0
                ?defaultTaskNum:traceDeleteSpoutNum);

        if(spout.contains("other")) {
            builder.setSpout(ComponentNameEnum.OTHER.getSpoutNameOther(), new OtherGdcp3Spout(props, false), otherSpoutNum == 0
