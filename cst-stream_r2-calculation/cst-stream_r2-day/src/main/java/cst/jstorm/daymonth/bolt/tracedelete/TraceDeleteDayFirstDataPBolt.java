 ...
 * @title
 */
public class TraceDeleteDayFirstDataPBolt extends BaseBasicBolt {
    ...
    public TraceDeleteDayFirstDataPBolt(Properties prop, boolean forceLoad) {
        this.prop = prop;
    ...
    public void prepare(Map stormConf, TopologyContext context) {
        ...
        logger = LoggerFactory.getLogger(TraceDeleteDayFirstDataPBolt.class);
    ...
    public void execute(Tuple tuple, BasicOutputCollector collector) {
        ...
        if(!StreamKey.TraceDeleteStream.TRACE_DELETE_DAY_BOLT_FIRST_DATA.equals(tuple.getSourceStreamId()))
            return;
        firstDataPersist(msg, StrategyChoose.generateStrategy(prop.getProperty(PropKey.DEAL_STRATEGY),
                httpUtils, prop.getProperty("url_base"), HttpURIUtil.TRACE_DELETE_DAY_SOURCE_SAVE,
                connection, HBaseTable.DAY_FIRST_ZONE.getTableName(),
                HBaseTable.DAY_FIRST_ZONE.getFirstFamilyName(),
                HbaseColumn.DaySourceColumn.traceDeleteDayColumns,
                TraceDeleteHourSource.class
        ),StreamTypeDefine.TRACE_DELETE_TYPE, CstConstants.TIME_SELECT.DAY);
    }
