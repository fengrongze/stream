...
import cst.jstorm.daymonth.calcalations.mileage.MileageMonthCalcBiz;
 ...
 * @Description mileage数据的月计算
 */
public class MileageMonthDataCalcBolt extends BaseBasicBolt {
    ...
    private MileageMonthCalcBiz mileageMonthCalcBiz;

    public MileageMonthDataCalcBolt(Properties prop, boolean forceLoad) {
        this.prop = prop;
    ...
    public void prepare(Map stormConf, TopologyContext context) {
        ...
        logger = LoggerFactory.getLogger(MileageMonthDataCalcBolt.class);
        ...
        mileageMonthCalcBiz = new MileageMonthCalcBiz();
    }

    @Override
    @SuppressWarnings("unchecked")
    public void execute(Tuple input, BasicOutputCollector collector) {
        if (!StreamKey.MileageStream.MILEAGE_MONTH_BOLT_F.equals(input.getSourceStreamId()))
            return;
        ...
        try {
            logger.debug("mileage month transfor data:{}", msg);
            ...
            GeneralAccumulationStreamExecution<MileageDayTransfor, MileageMonthTransfor, MileageMonthCalcBiz> generalStreamExecution = new GeneralAccumulationStreamExecution<>()
                    .createJedis(jedis)
                    .createSpecialCalc(mileageMonthCalcBiz)
                    .createSpecialSource(msg, StreamRedisConstants.MonthKey.MONTH_MILEAGE, DateTimeUtil.DEFAULT_DATE_MONTH);
            //如果从缓存中拿到不是空数据
            IHBaseQueryAndPersistStrategy<MileageMonthTransfor> iResultStrategy = StrategyChoose.generateStrategy(prop.getProperty(PropKey.DEAL_STRATEGY),
                    httpUtils, prop.getProperty("url_base"), HttpURIUtil.MILEAGE_MONTH_FIND,
                    connection, HBaseTable.MONTH_STATISTICS.getTableName(),
                    HBaseTable.MONTH_STATISTICS.getEighthFamilyName(), HbaseColumn.MonthStatisticsCloumn.mileageMonthColumns,
                    MileageMonthTransfor.class);
            ...
            if(CollectionUtils.isNotEmpty(persistValues))
                for (String str : persistValues)
                    collector.emit(StreamKey.MileageStream.MILEAGE_MONTH_BOLT_S, new Values(str, generalStreamExecution.gentMsgId()));
    ...
    public void declareOutputFields(OutputFieldsDeclarer declarer) {
        declarer.declareStream(StreamKey.MileageStream.MILEAGE_MONTH_BOLT_S, new Fields(new String[] {
