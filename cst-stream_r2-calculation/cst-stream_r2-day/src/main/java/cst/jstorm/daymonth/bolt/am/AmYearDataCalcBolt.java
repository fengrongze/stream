...
import cst.jstorm.daymonth.calcalations.am.AmYearCalcBiz;
 ...
 */
public class AmYearDataCalcBolt extends BaseBasicBolt {
    ...
    private AmYearCalcBiz amYearCalcBiz;

    public AmYearDataCalcBolt(Properties prop, boolean forceLoad) {
        this.prop = prop;
    ...
    public void prepare(Map stormConf, TopologyContext context) {
        ...
        logger = LoggerFactory.getLogger(AmYearDataCalcBolt.class);
        ...
        amYearCalcBiz = new AmYearCalcBiz();
    ...
    public void execute(Tuple input, BasicOutputCollector collector) {
        if (!StreamKey.AmStream.AM_YEAR_BOLT_F.equals(input.getSourceStreamId()))
            return;
        ...
        try {
            ...
            GeneralAccumulationStreamExecution<AmDayTransfor, AmYearTransfor, AmYearCalcBiz> generalStreamExecution =
                    new GeneralAccumulationStreamExecution<>()
                    .createJedis(jedis)
                    .createSpecialCalc(amYearCalcBiz)
                    .createSpecialSource(msg, StreamRedisConstants.YearKey.YEAR_AM, DateTimeUtil.DEFAULT_DATE_YEAR);
            //如果从缓存中拿到不是空数据
            IHBaseQueryAndPersistStrategy<AmYearTransfor> iResultStrategy = StrategyChoose.generateStrategy(prop.getProperty(PropKey.DEAL_STRATEGY),
                    httpUtils, prop.getProperty("url_base"), HttpURIUtil.AM_YEAR_FIND,
                    connection, HBaseTable.YEAR_STATISTICS.getTableName(),
                    HBaseTable.YEAR_STATISTICS.getThirdFamilyName(), HbaseColumn.YearStatisticsCloumn.amYearColumns,
                    AmYearTransfor.class);
            ...
            if(CollectionUtils.isNotEmpty(persistValues))
                for (String str : persistValues)
                    collector.emit(StreamKey.AmStream.AM_YEAR_BOLT_S, new Values(str, generalStreamExecution.gentMsgId()));
    ...
    public void declareOutputFields(OutputFieldsDeclarer declarer) {
        declarer.declareStream(StreamKey.AmStream.AM_YEAR_BOLT_S, new Fields(new String[] {
