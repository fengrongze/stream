...
import cst.jstorm.daymonth.calcalations.voltage.VoltageYearCalcBiz;
 ...
 * @Description voltage数据的年计算
 */
public class VoltageYearDataCalcBolt extends BaseBasicBolt {
    ...
    private VoltageYearCalcBiz voltageYearCalcBiz;

    public VoltageYearDataCalcBolt(Properties prop, boolean forceLoad) {
        this.prop = prop;
    ...
    public void prepare(Map stormConf, TopologyContext context) {
        ...
        logger = LoggerFactory.getLogger(VoltageYearDataCalcBolt.class);
        ...
        voltageYearCalcBiz = new VoltageYearCalcBiz();
    }

    @Override
    @SuppressWarnings("unchecked")
    public void execute(Tuple input, BasicOutputCollector collector) {
        if (!StreamKey.VoltageStream.VOLTAGE_YEAR_BOLT_F.equals(input.getSourceStreamId()))
            return;
        ...
        try {
            ...
            GeneralAccumulationStreamExecution<VoltageDayTransfor, VoltageYearTransfor, VoltageYearCalcBiz> generalStreamExecution = new GeneralAccumulationStreamExecution<>()
                    .createJedis(jedis)
                    .createSpecialCalc(voltageYearCalcBiz)
                    .createSpecialSource(msg, StreamRedisConstants.YearKey.YEAR_VOLTAGE, DateTimeUtil.DEFAULT_DATE_YEAR);
            //如果从缓存中拿到不是空数据
            IHBaseQueryAndPersistStrategy<VoltageYearTransfor> iResultStrategy = StrategyChoose.generateStrategy(prop.getProperty(PropKey.DEAL_STRATEGY),
                    httpUtils, prop.getProperty("url_base"), HttpURIUtil.VOLTAGE_YEAR_FIND,
            ...
            if(CollectionUtils.isNotEmpty(persistValues))
                for (String str : persistValues)
                    collector.emit(StreamKey.VoltageStream.VOLTAGE_YEAR_BOLT_S, new Values(str, generalStreamExecution.gentMsgId()));
    ...
    public void declareOutputFields(OutputFieldsDeclarer declarer) {
        declarer.declareStream(StreamKey.VoltageStream.VOLTAGE_YEAR_BOLT_S, new Fields(new String[] {
