 ...
 * @Description obd month persist
 * @title
 */
public class ObdMonthDataPersistBolt extends BaseBasicBolt {
    ...
    public ObdMonthDataPersistBolt(Properties prop, boolean forceLoad) {
        this.prop = prop;
    ...
    public void prepare(Map stormConf, TopologyContext context) {
        ...
        logger = LoggerFactory.getLogger(ObdMonthDataPersistBolt.class);
    ...
    public void execute(Tuple tuple, BasicOutputCollector collector) {
        ...
        if (StringUtils.isBlank(msg))
            return;
        if(!StreamKey.ObdStream.OBD_MONTH_BOLT_S.equals(tuple.getSourceStreamId()))
            return;
        resultPersist(msg, StrategyChoose.generateStrategy(prop.getProperty(PropKey.DEAL_STRATEGY), httpUtils,
                prop.getProperty("url_base"), HttpURIUtil.OBD_MONTH_SAVE,
                connection, HBaseTable.MONTH_STATISTICS.getTableName(),
                HBaseTable.MONTH_STATISTICS.getFirstFamilyName(),
                HbaseColumn.MonthStatisticsCloumn.obdMonthColumns,
                ObdMonthTransfor.class), StreamTypeDefine.OBD_TYPE, CstConstants.TIME_SELECT.MONTH);
    }
