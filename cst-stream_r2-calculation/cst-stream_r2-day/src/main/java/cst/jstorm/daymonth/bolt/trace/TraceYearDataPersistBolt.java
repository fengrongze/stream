 ...
 * @Description trace year persist
 * @title
 */
public class TraceYearDataPersistBolt extends BaseBasicBolt {
    ...
    public TraceYearDataPersistBolt(Properties prop, boolean forceLoad) {
        this.prop = prop;
    ...
    public void prepare(Map stormConf, TopologyContext context) {
        ...
        logger = LoggerFactory.getLogger(TraceYearDataPersistBolt.class);
    ...
    public void execute(Tuple tuple, BasicOutputCollector collector) {
        ...
        if (StringUtils.isBlank(msg))
            return;
        if(!StreamKey.TraceStream.TRACE_YEAR_BOLT_S.equals(tuple.getSourceStreamId()))
            return;
        resultPersist(msg, StrategyChoose.generateStrategy(prop.getProperty(PropKey.DEAL_STRATEGY), httpUtils,
                prop.getProperty("url_base"), HttpURIUtil.TRACE_YEAR_SAVE,
                ...
                TraceYearTransfor.class), StreamTypeDefine.TRACE_TYPE, CstConstants.TIME_SELECT.YEAR);
    }
