 ...
 * @title
 */
public class VoltageDayFirstDataPBolt extends BaseBasicBolt {
    ...
    public VoltageDayFirstDataPBolt(Properties prop, boolean forceLoad) {
        this.prop = prop;
    ...
    public void prepare(Map stormConf, TopologyContext context) {
        ...
        logger = LoggerFactory.getLogger(VoltageDayFirstDataPBolt.class);
    ...
    public void execute(Tuple tuple, BasicOutputCollector collector) {
        ...
        if(!StreamKey.VoltageStream.VOLTAGE_DAY_BOLT_FIRST_DATA.equals(tuple.getSourceStreamId()))
            return;
        firstDataPersist(msg, StrategyChoose.generateStrategy(prop.getProperty(PropKey.DEAL_STRATEGY),
                httpUtils, prop.getProperty("url_base"), HttpURIUtil.VOLTAGE_DAY_SOURCE_SAVE,
                connection, HBaseTable.DAY_FIRST_ZONE.getTableName(),
                HBaseTable.DAY_FIRST_ZONE.getFirstFamilyName(),
                HbaseColumn.DaySourceColumn.voltageDayColumns,
                VoltageHourSource.class
        ),StreamTypeDefine.VOLTAGE_TYPE, CstConstants.TIME_SELECT.DAY);
    }
