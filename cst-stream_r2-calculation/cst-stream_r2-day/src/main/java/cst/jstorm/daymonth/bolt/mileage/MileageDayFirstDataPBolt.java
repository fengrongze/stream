 ...
 * @title
 */
public class MileageDayFirstDataPBolt extends BaseBasicBolt {
    ...
    public MileageDayFirstDataPBolt(Properties prop, boolean forceLoad) {
        this.prop = prop;
    ...
    public void prepare(Map stormConf, TopologyContext context) {
        ...
        logger = LoggerFactory.getLogger(MileageDayFirstDataPBolt.class);
    ...
    public void execute(Tuple tuple, BasicOutputCollector collector) {
        ...
        if (!StreamKey.MileageStream.MILEAGE_DAY_BOLT_FIRST_DATA.equals(tuple.getSourceStreamId())) {
            return;
        }

        firstDataPersist(msg, StrategyChoose.generateStrategy(prop.getProperty(PropKey.DEAL_STRATEGY),
                httpUtils, prop.getProperty("url_base"), HttpURIUtil.MILEAGE_DAY_SOURCE_SAVE,
                connection, HBaseTable.DAY_FIRST_ZONE.getTableName(),
                HBaseTable.DAY_FIRST_ZONE.getFirstFamilyName(),
                HbaseColumn.DaySourceColumn.mileageDayColumns,
                MileageHourSource.class
        ), StreamTypeDefine.MILEAGE_TYPE, CstConstants.TIME_SELECT.DAY);
    }
