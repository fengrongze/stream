 ...
 * @Description mileage month persist
 * @title
 */
public class MileageMonthDataPersistBolt extends BaseBasicBolt {
    ...
    public MileageMonthDataPersistBolt(Properties prop, boolean forceLoad) {
        this.prop = prop;
    ...
    public void prepare(Map stormConf, TopologyContext context) {
        ...
        logger = LoggerFactory.getLogger(MileageMonthDataPersistBolt.class);
    ...
    public void execute(Tuple tuple, BasicOutputCollector collector) {
        ...
        if (StringUtils.isBlank(msg))
            return;
        if(!StreamKey.MileageStream.MILEAGE_MONTH_BOLT_S.equals(tuple.getSourceStreamId()))
            return;
        resultPersist(msg, StrategyChoose.generateStrategy(prop.getProperty(PropKey.DEAL_STRATEGY), httpUtils,
                prop.getProperty("url_base"), HttpURIUtil.MILEAGE_MONTH_SAVE,
                connection, HBaseTable.MONTH_STATISTICS.getTableName(),
                HBaseTable.MONTH_STATISTICS.getEighthFamilyName(),
                HbaseColumn.MonthStatisticsCloumn.mileageMonthColumns,
                MileageMonthTransfor.class), StreamTypeDefine.MILEAGE_TYPE, CstConstants.TIME_SELECT.MONTH);
    }
