 ...
 * @title
 */
public class AmYearDataPersistBolt extends BaseBasicBolt {
    ...
    public AmYearDataPersistBolt(Properties prop, boolean forceLoad) {
        this.prop = prop;
    ...
    public void prepare(Map stormConf, TopologyContext context) {
        ...
        logger = LoggerFactory.getLogger(AmYearDataPersistBolt.class);
    ...
    public void execute(Tuple tuple, BasicOutputCollector collector) {
        ...
        if (StringUtils.isBlank(msg))
            return;
        if(!StreamKey.AmStream.AM_YEAR_BOLT_S.equals(tuple.getSourceStreamId()))
            return;
        resultPersist(msg, StrategyChoose.generateStrategy(prop.getProperty(PropKey.DEAL_STRATEGY), httpUtils,
                prop.getProperty("url_base"), HttpURIUtil.AM_YEAR_SAVE,
                connection, HBaseTable.YEAR_STATISTICS.getTableName(),
                HBaseTable.YEAR_STATISTICS.getThirdFamilyName(),
                HbaseColumn.YearStatisticsCloumn.amYearColumns,
                AmYearTransfor.class), StreamTypeDefine.AM_TYPE, CstConstants.TIME_SELECT.YEAR);
    }
